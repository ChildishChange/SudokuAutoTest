...
namespace SudokuAutoTest
{
    class GitRepoHandler
    {
        ...
        public void GetRepoUrlFromBlog(string numberId)
        {
            ...
            {
                Logger.Error($"Student {_gitMapTable[numberId]}'s blog {_blogMapTable[numberId]} doesn't have a github repo, please check his blog again.", _loggerFile);
            }
        }
        ...
        public void CloneRepos()
        {
            ...
            foreach(var key in _repoMapTable.Keys)
            {
                try
                {
                    ...
                    {
                        Logger.Info($"Start Clone {key} !", _loggerFile);
                        CloneRepo(key);
                    }
                }
                catch (Exception e)
